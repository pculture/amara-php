...
class srtFile{
	 ...
	 */
	public function __construct($_filename, $_encoding = '', $string = null){
		...
		# Patch to allow loading from string.
		# Filenames shouldn't contain line-breaks,
		# subtitle strings always would.
		if ( $string || strpos( $_filename, "\n" ) !== false ) {
			$this->loadContentFromString();
		} else {
	...
	public function setFileContent( $srt ){
		$this->file_content = $srt;
	}
	 ...
	 */
	private function convertEncodingFromString(){
	...
	/**
	 * Loads content from a string and detect encoding if undefined
	 */
	private function loadContentFromString(){
		if(!($this->file_content = $this->filename))
				throw new \Exception( 'The string is not a proper SubRip track.');
		$this->file_content .= "\n\n"; // fixes string missing blank lines at the end
		$this->convertEncodingFromString();
	}
	...
	/**
	 * Returns the compiled SRT
	 *
	 * @param boolean $stripTags If true, use file_content_notag instead of file_content
	 */
	public function generateSRT( $stripTags = false ){
		$content = $stripTags ? $this->file_content_notag : $this->file_content;
		if(strtolower($this->encoding) != 'utf-8')
			$file_content = mb_convert_encoding($content, $this->encoding, 'utf-8');
		elseif(strtolower($this->encoding) == 'utf-8' && $this->has_BOM && substr($content, 0, 3) != UTF8_BOM)
